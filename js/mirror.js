...
function getPlaneMirror() {
  let geometry = new THREE.PlaneGeometry(WORLD_SIZE * 10, WORLD_SIZE * 10, 50, 50);
...
function addMirror() {
  mirror = getPlaneMirror();
  mirror.position.y = -230;
...
function updateMirror() {
  ...
  for (let i = 0; i < posArray.length; i += 3) {
    let x = posArray[i + 0];
    let y = posArray[i + 1];
    posArray[i + 2] = sin((x + y) * 0.20 + (frame) * 0.09) * 6;
  }
  mirror.geometry.attributes.position.needsUpdate = true;
}

/* global
scene frame WORLD_SIZE
